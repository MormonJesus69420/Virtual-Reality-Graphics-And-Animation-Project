...
bool HidBinding::operator==(const HidBinding& binding) const
{

  return (_action_name == binding._action_name) && binding.getInput()->operator==(*_input);
}

HidBindingCompare::HidBindingCompare(const HidInput& input)
    : _comp_input(true)
    , _text()
    , _input(input)
{
}
HidBindingCompare::HidBindingCompare(const QString text)
    : _comp_input(false)
    , _text(text)
    , _input(HidInput::getDefault())
{
}

bool HidBindingCompare::operator()(const HidBinding& binding) const
{
