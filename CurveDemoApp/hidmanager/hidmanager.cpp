    ...
    unsigned int custom_trigger)
{
  ...
  QSet<const HidAction*>::iterator item_itr = std::find_if(_hid_actions.begin(), _hid_actions.end(), HidActionCompareId(identifier));
...
bool HidManager::registerHidMapping(const QString& action_name, const HidInput* hid_input)
{
  ...
  HidActions::iterator act_itr = std::find_if(_hid_actions.begin(), _hid_actions.end(), HidActionCompareId(action_name));
  if (act_itr == _hid_actions.end())
    return false;
